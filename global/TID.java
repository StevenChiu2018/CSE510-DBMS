...
public class TID {
    ...
    public void getFromByteArray(byte[] array){

        this.numRIDs = Convert.getIntValue(0,array);
        this.position = Convert.getIntValue(4,array);
        int offset = 2*4;

        for (int i = 0; i < this.numRIDs; i++) {
            RID rid = new RID();
            this.recordIDs[i] = rid.getFromByteArray(array,offset+i*8);
        }
