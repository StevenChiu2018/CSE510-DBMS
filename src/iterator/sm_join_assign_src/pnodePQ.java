 ...
 * to add or remove elements from the tree.
 */
public abstract class pnodePQ {
   ...
   */
  abstract public void enq(pnode item) throws IOException, UnknowAttrType, TupleUtilsException;
   ...
   * @return <code>0</code> if the two are equal, <code>1</code> if <code>a</code> is greater,
   ...
   * { if (sort_order.tupleOrder == TupleOrder.Ascending) return pnodeCMP(a, b) <= 0; else if
   * (sort_order.tupleOrder == TupleOrder.Descending) return pnodeCMP(a, b) >= 0; else throw new
   * UnknowAttrType("error in pnodePQ.java"); }
   */
  /*
   * virtual pnode& front() = 0; // access min item virtual void del_front() = 0; // delete min item
   *
   * virtual int contains(pnode item); // is item in PQ?
   *
   * virtual void clear(); // delete all items
   *
   * virtual Pix first() = 0; // Pix of first item or 0 virtual void next(Pix& i) = 0; // advance to
   * next or 0 virtual pnode& operator () (Pix i) = 0; // access item at i virtual void del(Pix i) =
   * 0; // delete item at i virtual int owns(Pix i); // is i a valid Pix ? virtual Pix seek(pnode
   * item); // Pix of item
   *
